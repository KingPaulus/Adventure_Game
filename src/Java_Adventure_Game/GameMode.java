...
public class GameMode extends JPanel implements MouseListener, KeyListener {
	...
	public int BreiteX = Adventure_Main.BreiteMain;
	public int HoheY = Adventure_Main.HoheMain;
...
//	static int Levelgegner = 20;
	...
	int Schaden2 = 0;
	int SpielerIcon = 1;
	// 1Tod		2BogenSchütze		3Zauberer

	String GameNameNormal = Adventure_Main.GameName;
...
//		GameMode GM = new GameMode();
		...
		jf.setSize(Adventure_Main.BreiteMain,Adventure_Main.HoheMain);
		jf.setVisible(true);
		jf.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		jf.setTitle(Adventure_Main.GameName);
...
//			System.out.println("Schaden Bei dir = " + Schaden2);
	...
	public void paint(Graphics g) {
		...
		if (Restgegener <= 0) {
			System.out.println("Nächte Welle");
			spawnGegner = true;
...
//			Rechts
		...
		int großeDerHerzen = 20;
		g.setColor(Color.RED);
		Herzen = player.getHp() / 10;
//		System.out.println("Live = " + Herzen);

		for(int zzzu = 0;zzzu<Herzen;zzzu++) {
			g.drawImage(image5, NeueHerzenZeile, HoheY-180, großeDerHerzen, großeDerHerzen, null);
			NeueHerzenZeile = NeueHerzenZeile + (großeDerHerzen);
		}
		if (player.getHp() > 1 && player.getHp() < 10) {
			g.drawImage(image26, NeueHerzenZeile, HoheY-180, großeDerHerzen, großeDerHerzen, null);
		}
...
//		g.setColor(Color.BLACK);
		...
		if(waffe == 1) {
			...
			g.drawImage(image6, 235, 325, große, große, null);
		} else if(waffe == 2) {
...
//			g.fillRect(40, 80, 400, 50);
	...
	public void BogenSchützeSelectet () {
		WindowStart[1] = false;
		waffe = 3;
		player.setSpieler(60,15,50,25,40);
		System.out.println("BogenSchütze");
	}
